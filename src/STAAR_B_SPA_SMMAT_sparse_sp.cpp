...
// [[Rcpp::export]]
arma::vec STAAR_B_SPA_SMMAT_sparse_sp(arma::sp_mat G, arma::mat XW, arma::mat XXWX_inv, arma::vec residuals, arma::vec muhat, arma::mat weights_B, double tol, int max_iter,
double p_filter_cutoff, arma::sp_mat Sigma_i, arma::mat Sigma_iX, arma::mat cov)
{
	int i,k;
	double sum0 = 0.0;
    ...
    // calculate G_weights 
    arma::mat G_weights;
    G_weights.zeros(vn,wn);
	
	G_weights = G*weights_B;
	...
	arma::mat G_cumu;
	G_cumu.zeros(vn,wn);
	G_cumu = G_weights-XXWX_inv*(XW*G_weights);
