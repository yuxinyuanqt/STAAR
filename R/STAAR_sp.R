#' STAAR procedure using omnibus test (dgCMatrix version)
#'
#' The \code{STAAR_sp} function is a sparse matrix (dgCMatrix) version of \code{STAAR}.
#' It takes in a sparse genotype matrix, the object from fitting the null model,
#' and functional annotation data to analyze the association between a quantitative/dichotomous 
#' phenotype and a variant-set using the STAAR procedure.
#' For each variant-set, the STAAR-O p-value is a p-value from an omnibus test
#' that aggregates SKAT(1,25), SKAT(1,1), Burden(1,25), Burden(1,1), ACAT-V(1,25),
...
STAAR_sp <- function(genotype_sp,MAF=NULL,obj_nullmodel,annotation_phred=NULL,
